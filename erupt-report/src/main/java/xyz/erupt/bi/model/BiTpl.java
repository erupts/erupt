...
public class BiTpl extends MetaModelUpdateVo implements DataProxy<BiTpl> {
    ...
    @EruptField(
            views = @View(title = "路径"),
            edit = @Edit(title = "路径",
                    dynamicOn = @DynamicOn(dependField = "type", condition = "value == '" + BiTpl.TYPE_PATH + "'", match = DynamicOn.Ctrl.NOTNULL),
    ...
    @EruptField(
            ...
            edit = @Edit(title = "模板", desc = "语法Freemarker",
                    dynamicOn = @DynamicOn(dependField = "type", condition = "value == '" + BiTpl.TYPE_ONLINE + "'", match = DynamicOn.Ctrl.NOTNULL),
