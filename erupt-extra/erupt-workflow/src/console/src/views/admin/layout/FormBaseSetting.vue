    ...
    <el-form ref="baseSetting" :model="setup" label-position="top" label-width="80px">
      <el-form-item v-if="setup.logo" label="表单图标">
        ...
        <span class="change-icon">
          <span>
						<span>选择背景色</span>
						<el-color-picker v-model="setup.logo.background" :predefine="colors" show-alpha size="small"></el-color-picker>
					</span>
          <span>
            <span>选择图标</span>
						<el-popover placement="bottom-start" trigger="click" width="390">
							<div class="icon-select">
								<i v-for="(i, id) in icons" :key="id" :class="i" @click="setup.logo.icon = i"></i>
							</div>
      ...
      <el-form-item :rules="getRule('请输入表单名称')" label="表单名称" prop="formName">
        <el-input v-model="setup.formName" size="medium"></el-input>
      </el-form-item>
      <el-form-item :rules="getRule('请选择表单分组')" class="group" label="所在分组" prop="groupId">
        <el-select v-model="setup.groupId" placeholder="请选择分组" size="medium">
          <el-option v-for="(op, index) in fromGroup" v-show="op.groupId > -1" :key="index"
                     :label="op.groupName" :value="op.groupId"></el-option>
        </el-select>
        <el-popover placement="bottom-end" title="新建表单分组" trigger="click" width="300">
          <el-input v-model="newGroup" placeholder="请输入新的分组名" size="medium">
      ...
      <el-form-item label="表单说明">
        <el-input v-model="setup.remark" :autosize="{ minRows: 2, maxRows: 5}" maxlength="500" placeholder="请输入表单说明"
...
<!--      </el-form-item>-->
    ...
    <org-picker ref="orgPicker" :selected="select" multiple title="请选择可以管理此表单的人员" type="user" @ok="selected"></org-picker>
  </div>
</template>
