<template>
  <el-container>
    <el-header style="background: white">
      <layout-header v-model="activeSelect" @preview="preview" @publish="publishProcess"></layout-header>
    </el-header>
    ...
    <w-dialog v-model="validVisible" :showFooter="false" title="设置项检查">
      <el-steps :active="validStep" align-center finish-status="success">
        <el-step v-for="(step, i) in validOptions" :key="i" :description="step.description"
                 :icon="step.icon" :status="step.status" :title="step.title"/>
      </el-steps>
      <el-result :icon="validIcon" :subTitle="validResult.desc" :title="errTitle">
        <i v-if="!validResult.finished" slot="icon" class="el-icon-loading" style="font-size: 30px"></i>
        <div v-if="validResult.errs.length > 0" slot="subTitle" class="err-info">
          <ellipsis v-for="(err, i) in validResult.errs" :key="i + '_err'" :content="err" hover-tip>
            <i slot="pre" class="el-icon-warning-outline"></i>
          </ellipsis>
        </div>
        <template slot="extra">
          <el-button v-if="validResult.finished" size="medium" type="primary" @click="doAfter">
            {{ validResult.action }}
          </el-button>
        </template>
      </el-result>
