<template>
  <div>
    <el-form label-position="top" label-width="90px">
      <el-form-item class="user-type" label="选择触发的动作" prop="text">
      ...
      <div v-if="config.type === 'WEBHOOK'">
        <el-form-item label="请求地址" prop="text">
          <el-input v-model="config.http.url" placeholder="请输入URL地址" size="medium" >
            <el-select slot="prepend" v-model="config.http.method" placeholder="URL" style="width: 85px;">
        ...
        <el-form-item label="Header请求头" prop="text">
          ...
          <div v-for="(header, index) in config.http.headers" :key="header.name">
            ...
            <el-select v-if="header.isField" v-model="header.value" placeholder="请选择表单字段" size="small" style="width: 180px;">
        ...
        <el-form-item label="Header请求参数" prop="text">
          ...
          <div v-for="(param, index) in config.http.params" :key="param.name">
            ...
            <el-select v-if="param.isField" v-model="param.value" placeholder="请选择表单字段" size="small" style="width: 180px;">
              <el-option v-for="form in forms" :key="form.id" :label="form.title" :value="form.title"></el-option>
            </el-select>
        ...
        <el-form-item label="请求结果处理" prop="text">
          ...
          <div v-if="config.http.handlerByScript">
            <div>
              <span>请求成功😀：</span>
              <el-input v-model="config.http.success" :rows="3" type="textarea"></el-input>
            </div>
            <div>
              <span>请求失败😥：</span>
              <el-input v-model="config.http.fail" :rows="3" type="textarea"></el-input>
            </div>
          </div>
        </el-form-item>
      </div>
      <div v-else-if="config.type === 'EMAIL'">
        ...
        <el-form-item label="收件方" prop="text">
          <el-select v-model="config.email.to" allow-create default-first-option filterable multiple placeholder="请输入收件人" size="small" style="width: 100%;">
            <el-option v-for="item in config.email.to" :key="item" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="邮件正文" prop="text">
          <el-input v-model="config.email.content" :rows="4" placeholder="邮件内容，支持变量提取表单数据 ${表单字段名} " type="textarea"></el-input>
        </el-form-item>
      </div>
    </el-form>
  </div>
