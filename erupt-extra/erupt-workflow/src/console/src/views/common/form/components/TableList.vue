<template>
  <div>
    <div v-if="mode === 'DESIGN'">
      <draggable :list="_columns" :options="{animation: 300, chosenClass:'choose', sort:true}" class="l-drag-from"
                 group="form"
                 @end="drag = false" @start="drag = true; selectFormItem = null">
        <div v-for="(cp, id) in _columns" :key="id" :style="getSelectedClass(cp)" class="l-form-item" @click.stop="selectItem(cp)">
    ...
    <div v-else>
      <div v-if="rowLayout">
        <el-table :border="showBorder" :data="_value" :header-cell-style="{background:'#f5f7fa', padding:'3px 0'}" size="medium" style="width: 100%">
          <el-table-column fixed label="序号" type="index" width="50"></el-table-column>
          <el-table-column v-for="(column, index) in _columns" :label="column.title" :min-width="getMinWidth(column)" :prop="column.id">
            <template slot-scope="scope">
              <form-design-render v-model="_value[scope.$index][column.id]" :class="{'valid-error': showError(column, _value[scope.$index][column.id])}" :config="column" :mode="mode"/>
            </template>
          </el-table-column>
        ...
        <el-button icon="el-icon-plus" size="small" @click="addRow">{{placeholder}}</el-button>
      </div>
      <div v-else>
        <el-form v-for="(row, i) in _value" :key="i" :ref="`table-form-${i}`" :model="row" :rules="rules" class="table-column">
          <div class="table-column-action">
            <span>第 {{i + 1}} 项</span>
            <i class="el-icon-close" @click="delRow(i, row)"></i>
          </div>
          <el-form-item v-for="(column, index) in _columns" :key="'column_' + index" :label="column.title" :prop="column.id">
            <form-design-render v-model="row[column.id]" :config="column" :mode="mode"/>
          </el-form-item>
        </el-form>
        <el-button icon="el-icon-plus" size="small" @click="addRow">{{placeholder}}</el-button>
      </div>
