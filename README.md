...
## 什么是 Erupt
erupt 是一个低代码全栈框架，它使用 `注解` 配置来生成页面以及增删改查等数据功能，可减少开发工作量，极大提升效率。

## 为什么要做 Erupt ?
无论开发怎样的系统，都需要配套的管理后台做数据支撑，所以是非常重要的存在，但实际开发中存这无法规避的痛点，如：开发效率低下、UI界面不尽人意、交互凑合、代码重复、存在安全漏洞，导致其开发成本极高。
...
这个界面虽然用 Vue + Element UI + SSM 也能做出个大概，但仔细观察会发现它有大量细节功能如：
