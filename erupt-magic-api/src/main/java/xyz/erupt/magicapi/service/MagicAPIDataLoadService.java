	...
	public void run(String... args) {
		...
		new ProjectUtil().projectStartLoaded(menuKey, bool -> {
			if (bool) {
				...
				EruptMenu eruptMenu = eruptDao.persistIfNotExist(EruptMenu.class, new EruptMenu(menuKey, "接口配置", MenuTypeEnum.LINK.getCode(), magicAPIProperties.getWeb(),
						Integer.valueOf(EruptMenu.OPEN), sort.getAndAdd(10), null, null), "code", menuKey);
				...
				menus.forEach((key, value) -> {
					...
					eruptDao.persistIfNotExist(EruptMenu.class, new EruptMenu(menuCode, value, MenuTypeEnum.BUTTON.getCode(),
							key.name(), Integer.valueOf(EruptMenu.HIDE), sort.getAndAdd(10), null, eruptMenu), "code", menuCode);
				});
			}
		});
	}
