        ...
        new ProjectUtil().projectStartLoaded(menuKey, bool -> {
            if (bool) {
                ...
                EruptMenu eruptMenu = eruptDao.persistIfNotExist(EruptMenu.class,
                        new EruptMenu(menuKey, "接口配置", EruptTplService.TPL, MagicApiTpl.MAGIC_API_PERMISSION,
                                MenuStatus.OPEN.getValue(), sort.getAndAdd(10), "fa fa-cube", null), "code", menuKey);
                ...
                menus.forEach((key, value) -> {
                    ...
                    eruptDao.persistIfNotExist(EruptMenu.class, new EruptMenu(menuCode, value, MenuTypeEnum.BUTTON.getCode(),
                            MAGIC_API_MENU_PREFIX + key.name(), MenuStatus.HIDE.getValue(),
                            sort.getAndAdd(10), null, eruptMenu), "code", menuCode);
                });
            }
        });
    }
