...
public class CsvCodec implements FileCodec {
    ...
    /**
     * Field delimiter; subclasses override it to reuse the RFC 4180 quoting engine
     * for other single-char separators (e.g. TSV's tab).
     */
    ...
    public String encode(EruptModel eruptModel, EruptFile eruptFile, List<JsonObject> records) {
        ...
        StringBuilder csv = new StringBuilder(String.join(String.valueOf(delimiter), columns)).append('\n');
        for (JsonObject json : records) {
            for (int c = 0; c < columns.size(); c++) {
                if (c > 0) csv.append(delimiter);
    ...
    private String escape(String value) {
        if (value.indexOf(this.delimiter()) >= 0 || value.contains("\"") || value.contains("\n") || value.contains("\r")) {
