...
public class FileCodecTest {
    ...
    @Test
    void tsvSplitsOnTabsWithQuoting() {
        List<JsonObject> rows = new TsvCodec().decode("name\tnote\nfoo\t\"a\tb\"\nbar\tplain", list);
        assertEquals(2, rows.size());
        assertEquals("a\tb", rows.get(0).get("note").getAsString());
        assertEquals("plain", rows.get(1).get("note").getAsString());
    }

    @Test
    void jsonlReadsOneObjectPerLine() {
        JsonlCodec codec = new JsonlCodec();
        List<JsonObject> rows = codec.decode("{\"id\":1}\n\n{\"id\":2}\n", list);
        assertEquals(2, rows.size());
        assertEquals(2, rows.get(1).get("id").getAsInt());
        // encode keeps each record on its own line
        assertEquals(2, codec.encode(null, list, rows).trim().split("\n").length);
    }

    @Test
    void iniGroupsSectionsIntoNestedObjects() {
        IniCodec codec = new IniCodec();
        assertTrue(codec.singleton(single));
        JsonObject row = codec.decode("name=app\n; a comment\n[db]\nhost=localhost\nport=5432", single).get(0);
        assertEquals("app", row.get("name").getAsString());
        assertEquals("localhost", row.getAsJsonObject("db").get("host").getAsString());
        assertEquals("5432", row.getAsJsonObject("db").get("port").getAsString());
    }
