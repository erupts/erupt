...
/**
 * Skeleton for data sources that can enumerate their rows as beans (or maps):
 * subclasses supply data via {@link #data}, and searching, drill condition-string
 * parsing, sorting and paging are all evaluated here with the same semantics as
 * the persistent implementations. Write operations default to a read-only error.
 * <p>
 * Subclasses override what their backend can do natively: {@code queryList} /
 * {@code findDataById} for query push-down, the write methods for real writes.
 * A subclass whose {@link #data} already applies the query's conditions at the
 * source must return {@code true} from {@link #conditionsPushedDown()} so the
 * conditions are not re-evaluated here with subtly different semantics.
 * <p>
...
public abstract class EruptBeanDataService<T> implements IEruptDataService {
    ...
    /**
     * Whether {@link #data} already filtered the beans by the query's conditions
     * at the source. When {@code true} the base class skips its in-memory
     * re-evaluation, avoiding double filtering.
     */
    ...
    public void addData(EruptModel eruptModel, Object object) {
        throw new EruptWebApiRuntimeException(I18nTranslate.$translate("data.read_only"));
    }
    ...
    private List<Map<String, Object>> rows(EruptModel eruptModel, EruptQuery eruptQuery) {
        List<Condition> conditions = this.conditionsPushedDown()
                ? Collections.emptyList() : this.mergeConditions(eruptQuery);
